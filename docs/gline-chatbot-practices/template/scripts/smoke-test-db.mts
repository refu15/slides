...
const q = sql()
const salt = process.env.SESSION_SALT ?? 'test-salt-12345'
...
console.log('[OK] session hash:', (await hashSessionId('browser-uuid-abc', salt)).slice(0, 16) + '...')
...
if (process.env.APPLICANT_ENC_KEY) {
  const id = await insertApplicant(q, {
    name: 'テスト太郎',
  ...
  const row = await getApplicant(q, id)
...
const sid = await hashSessionId('smoke-session', salt)
await recordEvent(q, { sessionId: sid, type: 'chat_open' })
await writeConversation(q, { sessionId: sid, turnIndex: 0, role: 'user', content: 'テスト smoke@example.com' })
...
if ((nChunks[0] as any).n > 0 && process.env.GEMINI_API_KEY) {
  const hits = await search(q, '御社の強みは？', {}, 3)
