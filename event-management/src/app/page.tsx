...
export default function HomePage() {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        const { getCurrentUser } = await import("@/lib/auth/google-auth");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="max-w-md w-full bg-white shadow-2xl rounded-2xl p-8 space-y-6 border-2 border-black">
          ...
          {user ? (
            <div className="space-y-4">
              <div className="text-center p-4 bg-green-50 text-green-700 rounded border border-green-200">
                <p className="font-bold text-sm">ログイン済みです</p>
                <p className="text-xs">{user.email}</p>
              </div>
              <Button
                onClick={() => router.push('/dashboard')}
                className="w-full h-12 bg-black text-white hover:bg-gray-800 border-2 border-black rounded-lg font-semibold"
              >
                ダッシュボードへ進む
              </Button>
            </div>
